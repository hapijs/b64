...
exports.Encoder = internals.Encoder = function () {
    ...
    this._reminder = internals.blank;
};
...
internals.Encoder.prototype._transform = function (chunk, encoding, callback) {
    ...
    var part = this._reminder.length ? Buffer.concat([this._reminder, chunk]) : chunk;
...
/*
    ...
    var encodeChars = internals.encodeChars;
    var len = buffer.length;
    var allocated = Math.ceil(len / 3) * 4;
    var result = new Buffer(allocated);
    ...
    for (var i = 0, j = 0; i < len;) {
        ...
        if (i === len) {
            result[j++] = encodeChars[c1 >> 2];
            result[j++] = encodeChars[(c1 & 0x3) << 4];
            result[j++] = '=';
            result[j++] = '=';
            result[j++] = '=';
            break;
        ...
        if (i === len) {
            result[j++] = encodeChars[c1 >> 2];
            result[j++] = encodeChars[((c1 & 0x3) << 4) | ((c2 & 0xF0) >> 4)];
            result[j++] = encodeChars[(c2 & 0xF) << 2];
            result[j++] = '=';
            break;
        }
        ...
        result[j++] = encodeChars[c1 >> 2];
        result[j++] = encodeChars[((c1 & 0x3) << 4) | ((c2 & 0xF0) >> 4)];
        result[j++] = encodeChars[((c2 & 0xF) << 2) | ((c3 & 0xC0) >> 6)];
        result[j++] = encodeChars[c3 & 0x3F];
    }
